...
namespace Business.Services {
    public class StreamService : IStreamService {
        ...
        public async Task<IResponse<StreamListDto>> Update(StreamListDto dto) {
            var updatedEntity = await _unitOfWork.GetRepository<Entities.Concrete.Stream>().Find(dto.Id);
            _unitOfWork.GetRepository<Entities.Concrete.Stream>().Update(_mapper.Map<Entities.Concrete.Stream>(dto), updatedEntity);
            await _unitOfWork.SaveChanges();
            return new Response<StreamListDto>(ResponseType.Success, dto);
        }

        public async Task<IResponse> Remove(long id) {
            var removedEntity = await _unitOfWork.GetRepository<Entities.Concrete.Stream>().GetByFilter(x => x.Id == id);

            if (removedEntity != null) {
                _unitOfWork.GetRepository<Entities.Concrete.Stream>().Remove(removedEntity);
                await _unitOfWork.SaveChanges();
