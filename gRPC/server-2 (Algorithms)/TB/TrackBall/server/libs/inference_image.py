...
batch_size = 1
classes = 1
scaling = "NONE" #['NONE', 'INCEPTION', 'VGG']
url = 'localhost:8001'
...
def requestGenerator(input_name, output_name, c, h, w, format, dtype, result_filenames, supports_batching):
    
    output = service_pb2.ModelInferRequest().InferRequestedOutputTensor()
    output.name = output_name
    output.parameters['classification'].int64_param = classes
    ...
    if format == mc.ModelInput.FORMAT_NHWC: # NCHW Format (batch N, channels C, height H, width W)
        input.shape.extend(
            [batch_size, h, w, c] if supports_batching else [h, w, c])
    else:
        input.shape.extend(
            [batch_size, c, h, w] if supports_batching else [c, h, w])
    ...
    for filename in filenames:
        ...
        image_data.append(preprocess(img, format, dtype, c, h, w,scaling))
...
if __name__ == '__main__':
    ...
    max_batch_size, input_name, output_name, c, h, w, format, dtype = parse_model(metadata_response, config_response.config)
    
    # N-C-H-W (batch-channel-height-width)
    # 0, input_1, acdtivation_18, 9, 360, 640, 2, FP32,
    ...
    if streaming:
        for response in grpc_stub.ModelStreamInfer(
                requestGenerator(input_name, output_name, c, h, w, format, dtype, result_filenames, supports_batching)
        ):
            responses.append(response)

    else:
        for request in requestGenerator(input_name, output_name, c, h, w, format, dtype, result_filenames, supports_batching):
    ...
    for response in responses:
        if streaming:
            if response.error_message != "":
                ...
                postprocess(response.infer_response, result_filenames[idx], batch_size, supports_batching)
        else:
            postprocess(response, result_filenames[idx], batch_size, supports_batching)
        idx += 1