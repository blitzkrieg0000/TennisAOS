...
class WorkManager():
    ...
    def CourtLineWorks(self, BYTE_FRAMES_GENERATOR, data):
        courtLines = None
        ...
        return courtLines, self.court_warp_matrix, canvas


    def CropBottomCourt(self, byte_frame):
        tempFrame  = Converters.Bytes2Frame(byte_frame)
        mask = cv2.warpPerspective(self.white_mask, np.array(self.court_warp_matrix), tempFrame.shape[1::-1])
    ...
    def PrepareAlgorithms(self, **kwargs):
        threadSubmits = {
            self.ThreadExecutor.submit(self.trackBallClient.findTennisBallPosition, kwargs["byte_frame"], kwargs["data"]["topicName"]) : "DetectBall",
            self.ThreadExecutor.submit(self.bodyPoseClient.ExtractBodyPose, Converters.Frame2Bytes(kwargs["croppedBottomCourtFrame"])) : "BodyPose"
        }
    ...
    def ProcessData(self, data):
        ...
        #! 3-DETECT_COURT_LINES
        courtLines, self.court_warp_matrix, canvas = self.CourtLineWorks(BYTE_FRAMES_GENERATOR)

        #! 4-ALGORITHMS (DETECTION)
        for i, consumerResponse in enumerate(BYTE_FRAMES_GENERATOR):
            
            #PreRequest
            croppedBottomCourtFrame = self.CropBottomCourt(consumerResponse.data)
            settings = {
                "data" : data,
                "byte_frame" : consumerResponse.data,
                "croppedBottomCourtFrame" : croppedBottomCourtFrame
            }

            # PREPARE SUBMIT
            threadSubmits =self.PrepareAlgorithms(**settings)
        ...
        self.SaveResults(self, data, processedAOSData, canvas, ball_fall_array, all_body_pose_points, all_ball_positions)
